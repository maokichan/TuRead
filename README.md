...
> 笔记与划线 / 跨书笔记管理 / 主题与阅读参数）。**云端同步服务端已实现（v0.2.0），
> 但客户端尚未与真实服务器打通** —— 也就是说：现在拿到的客户端是**单机阅读器**。
...
| **沉浸式阅读** | 全屏的是"桌"，正文是居中的"纸"；阅读页零控件（退出 `Esc`）；一条**挂载线**贯穿页面：左段垂挂**目录 / 笔记**、右段垂挂**阅读参数** |
...
| **房间（半成品）** | 房间界面与协议都在，但**尚未与真实服务器打通** —— 见 [`TODO.md`](TODO.md) |

**快捷键**（阅读器内）：`←/→`、`PgUp/PgDn`、`Space`（分页模式）、`t` 目录、`p` 阅读参数、
`F11` **进入**全屏（退出全屏用 `Esc`）、鼠标侧键翻页、分页模式下点击左右区域翻页。
