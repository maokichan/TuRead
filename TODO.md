...
- [ ] **(用户登记，明确"不要 one-shot") 阅读参数的剩余部分**（依据 `client/docs/STYLE.md` §5.9 的频率表）；
  ⚠ **2026-09-12 降速（LANDSCAPE §6 判断 2"暴露的参数是负担"）**：以下不再按原计划自动落右侧面板，
  需要时逐个评估是否值得进入"默认暴露集"：
...
- [ ] **多书架 / 书库容器（⚠ 2026-09-12 冻结重审，LANDSCAPE §6 判断 1"阅读器=索引，不做实体管理"）**：
  分类/容器/标签系统属外部数字资产管理器（用户的 TagHit）职责，TuRead 自建容器 = 重走 Calibre 混合形态的笨重路线。
  远期正确形态更可能是**互通**（TuRead 订阅 TagHit 的工作区/标签作为书籍过滤器），而非自建容器模型。
  原讨论存档（2026-09-09 群聊重新定界）：当前只有单一书库 + 列表/网格两种排列，
