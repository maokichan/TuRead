...
- [ ] **已知问题（2026-09-01）：App 集成侧 EPUB 正文仍空（定位中）**：harness（无 CSP 独立页）同调用序渲染正常（1111 字），App 内 `renderTo → record() → goToChapterIndex(0)` 后 iframe 内 `bodyHtml=764`（布局壳）但正文空、无报错。已排除：初始导航顺序、StrictMode 双调用（改用模块级防重入 `devAutoOpened`）。当前怀疑 kookit `getDocument()` 硬编码 `#page-area` 在 App 里定位的元素与渲染目标不一致（探针 `pageAreaSame` 已写好待跑）。详见 `client/docs/KOOKIT.md` §8 / `client/docs/RENDER_INTERFACE.md` §8。另：PDF 真机翻页待验（无头验证下 next 后位置仍第0页，异步 canvas 渲染时序）
