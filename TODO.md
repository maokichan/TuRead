...
- [x] **(2026-09-12 二次定位并修复) 阅读页 → 书库"闪出第一本封面"**（仅网格模式）：用户定位"闪的是
  可视窗口第一本书、只在网格模式"→ 根因 = `useVirtualRange` 在组件隐藏（display:none）期间
  ResizeObserver 上报 0×0，网格列数被算成 1，切回书库的**首帧**按单列渲染（第一本封面撑满整行），
