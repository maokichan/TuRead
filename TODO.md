...
- [x] **(P1) 封面/元数据解析下放**（2026-09-12 完成）：kookit `getMetadata` 全书解析移到**离屏解析窗口**
  （隐藏 BrowserWindow = 独立进程：`parse.html` 双入口 + main 中继 + `IMetadataExtractor` 端口）；
  政策配套：失败负缓存（`coverFailed`）+ 启动补封面延迟 20s。328 本实测：提取期间主窗口 CPU~10%，
  积压 235 本一轮收敛（180 成功 / 55 负缓存）。**余债**：导入的整文件 IPC 传输待优化
  （指纹虽只哈希 192KB 采样，但文件得整个读进渲染层）——大文件夹导入慢的根源。
- [x] **(P1) 书库窗口化渲染**（2026-09-12 完成）：全量渲染几百条目卡顿（用户定"不要一次性渲染"）。
  `useVirtualRange`（自研 hook，无新依赖）：列表/网格只挂载可视区 ±4 行，上下占位撑出总高；
  网格列数按容器宽计算（与原 auto-fill minmax(118px) 同口径）、BookTile 标题区定高两行（行高可计算）。
  328 本实测：滚动/切视图/详情抽屉均流畅。
- [x] **(P1) TXT 兼容修复**（2026-09-12 完成）：**此前 TXT 全格式打不开**——kookit TxtRender 渲染路径
  `new TextDecoder(config.charset)` 要求调用方给编码，适配器传 `''` 直接 RangeError。
  修法：`detectTextCharset`（chardet，MIT，首 4KB 采样，标签映射 ASCII→utf-8 等）在 `open()` 与
  离屏解析页两处注入。实测 GBK（GB18030）/UTF-8+BOM 均渲染 OK 且正确分章（3 章）。
- [x] **(P1) MD 渲染**（2026-09-12 验证）：MD 走 marked→html，渲染无错；**目录只认 h1**（kookit
  `makeHtmlBook` 只查 `h1,title`，h2+ 不进目录）→ 归入「自建目录」。
...
- [ ] **(2026-09-12 用户提出) 阅读器沉浸全屏**：阅读态可**隐藏顶部标题栏**（真全屏）——
  ① 设置页给开关（默认关）；② 快捷键即时切换（候选 F9/F11，归属「键鼠意图层」统一裁决）；
  隐藏标题栏后窗口拖拽/控制键入口消失 → 需要贴缘召唤或快捷键找回，与上一条的新手提示同族。
...
- [ ] **(待复现) 阅读页 → 书库"闪出一本书的封面"**（2026-09-12 用户复报：抽屉清理修复**未命中**）：
  首修（抽屉状态）不是根因或不是唯一根因。剩余候选：① 库视图重新可见时 blob 封面图**异步解码
  pop-in**；② 视图切换闪没动画（--flash-bg/负片三角）被误认；③ FittedTitle 首帧量测跳变。
  下次出现时请记录：操作序列 + 出现位置（全屏？卡片位置？哪本书的封面）+ 截图；必要时加
  慢速动画/元素轮廓的 dev 探针定位。
...
- [ ] **(高优 · 当前主目标) 数据持久化 / 笔记划线落地**（2026-09-12 用户定为主要目标；同日二次批复
  ...
  **实施第一步 = better-sqlite3 打包 spike**（开放问题 1，成败决定走原生还是 wasm）。
