...
- [ ] **kookit 渲染集成（v0.1.1，遗留修复 2026-09-01 已落地①②③⑤ + PDF 注入）**：适配器已从桩换真实（vendor 单文件 ESM + readFile 注入 + `#page-area` 契约已修 + 阅读视图 + dev 无头验证）。**单体 harness 自检已强化并全绿（2026-09-06，EPUB/MOBI/AZW3 含"翻页位置必须变化"断言，见 `client/docs/KOOKIT.md` §9）**——单体侧无阻塞。**App 侧重要结论（2026-09-06）**：kookit 文字类渲染不监听宿主 scroll，`next()` smooth 滚动刚开始就 `record()` 算旧位置，**App 滚动/翻页后须等滚动停稳补一次 `record()`**（PDF 例外，自带 scroll 监听）；宿主容器必须 `overflow-y:auto`（§5.10）
- [ ] **已知问题（2026-09-01）：App 集成侧 EPUB 正文仍空（定位中）**：harness（无 CSP 独立页）同调用序渲染正常，App 内 `renderTo → record() → goToChapterIndex(0)` 后 iframe 内 `bodyHtml=764`（布局壳）但正文空、无报错。已排除：初始导航顺序、StrictMode 双调用（改用模块级防重入 `devAutoOpened`）。**2026-09-06 单体自检强化后进一步收窄嫌疑**：单体侧（含更严格断言）无任何问题，kookit 本体可基本排除，嫌疑集中在 App 集成环境（CSP/容器结构/`#page-area` 定位不一致——探针 `pageAreaSame` 已写好待跑）。详见 `client/docs/KOOKIT.md` §8 / `client/docs/RENDER_INTERFACE.md` §8
...
- [ ] **PDF 支持（2026-09-01 实装主体完成）**：pdfjs-dist@4.8.69 注入（adapter 动态加载 vendor + `ensurePdfjs`）+ `/lib/pdfjs/` 静态资源（cmaps/standard_fonts/2 个 css/worker）+ CSP worker-src；无头验证 PDF 渲染 OK（19 页、canvas、subHtml）。页码定位不依赖 OCR；扫描版走 `external-engine` 插槽接本地 OCR（PP-OCRv5 等，单独立项，接口见 `client/docs/RENDER_INTERFACE.md` §6）。OCR 多端一致性见 `KOOKIT.md` §8.2。**剩余**：① App 真机交互（翻页/进度）验证 ② 单体 harness 侧（2026-09-06）：pdfjs 注入 + `/lib/pdfjs/` serving 已打通（19 页容器创建、无报错）但 **canvas 未渲染**，待定位 `renderPdfPage → handleRenderPDFChapter → section.load()/render()` 链路断点，且自检断言需按 PDF 语义特判（innerText 恒 0，查 canvas 数 + 页位置）——优先级低，App 侧 PDF 验证优先
