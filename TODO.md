...
- [x] **(2026-09-12) 返回书库"闪出上一本的封面"修复**：根因 = 详情抽屉是常驻挂载的组件状态，
  离场路径不清理（openBook 路径 v0.1.9 清了，侧边栏离场没清）→ 离开书库即收起抽屉（`activeFeature`
  副作用）。若用户仍观察到"闪封面/闪内容"的其他形态（如封面图重解码闪烁），下次出现时记录规律再立项。
