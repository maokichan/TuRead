...
- [ ] **(P1) 封面/导入的主线程解析下放**（2026-09-12 事故修复后的余债）：kookit `getMetadata`（全书解析）
  与 canvas 缩略图都在**渲染主线程**跑，书库上规模后（328 本实测）单本秒级阻塞 × 批量 = UI 长时间饿死。
  已落的政策修复（同日）：失败负缓存（`coverFailed`）+ 启动补封面延迟 20s + 每本之间让出主线程。
  **治本方向**：解析移出主线程（Worker / 主进程），导入的整文件 IPC 传输同样待优化
  （指纹虽只哈希 192KB 采样，但文件得整个读进渲染层）。
