...
- [ ] **(专项) 深色主题显示问题**（2026-09-09 记录，用户指定单独立项）：暗色下多处文字可见性有问题。
  已知两类：① 视图切换动画的"深色块"与页面底色几乎同色（`--flash-bg` 已改为反色块，**其余动效待复核**）；
  ② **阅读器内部分文字被吞没**（现象待定位：候选原因是 kookit iframe 内容色 / 宿主
  `--page-bg`·`--page-text` / EPUB 自带深色样式三者叠加）。
  做法：先做全 token 对比度审计（四套取向 × 全部文字色）+ 阅读页实测，再统一修；
...
### 从各文档誊挪过来的待办（2026-09-09 文档收拢，原处改为指向本文件）
...
- [ ] **渲染层验收断言待补**（`client/docs/STYLE.md` §8）：
  ① `dev/selfCheck.ts` 增「无边框文字按钮存在性」与「字号白名单扫描」两条断言
