...
- [ ] **(用户登记，明确"不要 one-shot") 阅读页的控件面与预留**（2026-09-11 定，依据 `client/docs/STYLE.md` §5.9）：
  - **显示设计控件**：页面宽度 / **段宽度** / 字号 / 行距 —— 用户定"要考虑不同人的屏幕与字体大小，
    这些远期都应可自由调整，乃至**内容区内一行的长度**"。当前只落了**固定档位**
    （閱讀寬度 窄 620 / 中 760 / 寬 920），落库形态是 **px 数值**（`readerSettings.readerWidth`）
    而不是档位名，自由调节 UI 暂时隐藏（未来接滑杆/自适应不用改数据模型）。
  - **右侧"模型阅读"容器（预留）**：为未来的模型阅读面板预留位置 —— 布局必须保住
    "中央正文列 + 两侧可增长留白"这个结构，未来面板只占右侧留白，不挤压正文列语义。
...
- [ ] **(专项) 深色主题显示问题**（2026-09-09 记录，用户指定单独立项）：暗色下多处文字可见性有问题。
  ...
  > 进度（2026-09-11）：**「阅读器内部分文字被吞没」的"深色注入吞正文"假设已销案** ——
  > 隔离测试（同书同冷态、浅/深背靠背）显示两侧 `bodyHtml=704`、`正文textContent=109` **完全一致**，
  > 症状来自把 `innerText`（排版相关，实测 70/109 漂移）当"正文长度"用；已改为 `textContent` 判定，
  > 并新增「注入前后正文不许变」断言。**全 token 对比度审计仍未做**（本专项的其余部分）。
