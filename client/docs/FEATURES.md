...
> 用户需求：进入阅读 = 专注文本，不必要内容全部退场（koodo 式）。**视觉细则一律以 `STYLE.md` §5.8 为准**。
> 契约：`CONTRACTS.md` v0.3.0（`IRenderService.applyTheme` / `RenderOptions.theme` 消费 / `core/domain/theme.ts`）
> + v0.3.1（`ReaderSettings.readerWidth`）。
> ⚠ **2026-09-11 二次修订（用户纠正）**：上一版把"沉浸"做成了"正文铺满全屏 + 一条临时顶栏" ——
> **全屏的是"纸"不是"正文"**，且**阅读页上不得有任何栏**（浮动出来的栏同样是状态栏）。

**布局**
- 外层**全屏"桌"**容器（`--desk-bg`）+ 内层**居中定宽"纸"**（`--read-width`，档位 620/760/920，默认 760）；
  纸有 **1px `--page-edge` 边** —— 内容与背景**靠颜色区分**（`STYLE.md` §5.8 v0.5），不用阴影/圆角。
- 宿主容器 `#page-area` **就是纸（正文列）**（kookit 硬契约；`clientWidth` 决定排版宽度，见 `KOOKIT.md` §5）；
  外层桌容器把滚轮事件转发给列，保证鼠标停在两侧留白也能滚。
- 滚动条 4px、静息几乎不可见（指针进入正文列才微现）——不得影响注意力。
- `AppShell`：`activeFeature === 'reader'` 时**侧边栏隐藏**（退场），**贴左缘按钮**点击召回图标栏
  （可见条 6×160，判定区 18×200 外扩一圈）；**阅读态的侧边栏是覆盖式**（`absolute` 浮在左侧桌边，**不占布局**）
  → **打开左侧菜单不影响阅读器界面宽度**。⚠ 只给阅读态：其他功能态的交互形态需求未知，保持占位（`STYLE.md` §5.8 v0.7）。
  侧边栏宽度 = `--sidebar-w`（与目录几何共用同一 token）。
...
**主题模型与夜间模式**（见 `STYLE.md` §3.4 + `CONTRACTS.md` §2）- **2 主题 × 2 模式**：纯色/羊皮纸（tone）× 深/浅（mode）；数据层沿用四值。
- **正文页跟随模式**：深色模式 = 该主题的深色纸；非 PDF 正文深色由 `IRenderService.applyTheme` 注入
  （kookit `setStyle` 注入口，PDF 是位图不走此路、单独立项）。

**不做的（本轮）**
- PDF 像素反相（夜间模式第二部分，后置）；阅读器字号/行距/字体设置项（kookit config 注入方案未定，见 TODO）。
- **用户已登记但明确"不要 one-shot"的预留**（`STYLE.md` §5.9，只登记不实现）：显示设计控件（页面宽度/段宽度/字号/行距）、
  **右侧"模型阅读"容器**、**同步相关控件**。布局结构（中央正文列 + 两侧可增长留白）必须为此留出余地。
