...
| **NotesFeature** | `store` 笔记**读模型**（`listAllNotes` / `countAllNotes`，见 `CONTRACTS.md` §4.4）；跳转 = `host.openReader(editionId, { revealNoteId })`；複製 = `clipboard.writeText`（§4.6） | **无**（纯查询，不持有跨功能状态） | **无**（**激活时重读** —— 它与阅读器**不同屏**，故不会陈旧；见 §12） |
...
- **交互语义**：单击 → 右侧**详情抽屉**；双击 → 打开；键盘 Enter 打开、Space 详情、Delete 移除；
  ...
  右键书 → 菜单「打開 / 詳情 / 從書庫移除」（资源管理器语义，2026-09-13）；书可**拖拽**（拖到書箱 = 移入）。
- **层级浏览与書箱（2026-09-13 用户定稿：资源管理器式）**：书架 = 层级结构（不再只是平面全库）。
  **書箱 = 与书籍同地位的一等条目**——外观、选中、右键、拖拽、更名全部对齐书籍，唯一区别是
  "打开 = 进入下一层"。交互全部对齐文件资源管理器：**单击选中、双击进入、Enter 进入、F2 更名
  （Enter/失焦提交，不用 Esc——全局键义归意图层）、右键菜单（新建子書箱/更名/移動到/移除）、
  拖书入箱 = 移动（单亲归属，`moveBookToContainer`）**、拖到状态栏面包屑段 = 移动到该层（拖到根 = 移出，
  拖入段高亮提示）。
  **移动全量交互（2026-09-13 二轮）**：書箱本身也是**拖拽源**（`moveContainer`，拖箱入箱 / 拖到面包屑段，
  防成环由后端拒绝）；书与書箱右键均有**「移動到」子菜单**（上一層〔可出書箱〕/ 根層 / 当前层各兄弟書箱；
  `ContextMenu` 支持一层子菜单）。同层原地放 = 无操作（parentId 没变不打库）。
  右键空白处（自建模式）→「新建書箱」= 立即建出「新建書箱」并直接进入行内更名。
  状态栏右端显示**当前层级**面包屑（根 = 库名；自建库路径名 = 書箱名；映射库 = 文件夹名），可点回跳。
  ...
  **层级后退/前进（2026-09-13 用户定，对齐文件资源管理器）**：历史栈在 `LibraryFeature`
  ...
  两种模式（建库时二选一；**术语与代码值已按 v0.4.0 统一** —— 见 `DATA_MODEL.md` §6.2）：
  ① **映射库**（`mode='mapped'`，原"虛擬映射"）：跟踪唯一真实文件夹，层级 = 真实文件夹树，
  "書箱"即文件夹；⚠ **不能在里面导入**——书只能在真实路径上加，靠**扫描**对账（§6.1）；
  ② **自建库**（`mode='curated'`，原"自建書箱"）：空库起步，用户自建書箱树，书靠导入。
...
- **分页模式滚轮翻页（2026-09-13 用户定）**：默认键盘、不提供按钮控件，鼠标滚轮可翻页（像 koodo）。
  滚轮接管在**整个阅读器区**（含左右点击翻页带——那两条带压在纸面上方，挂在纸上会漏；
  目录/参数面板内部让位），正文 iframe 内部走 iframe 桥；平滑滚轮短窗累积 + 过阈值翻一页
  + 冷却窗防连翻；scroll 模式不拦（iframe 内原生滚动自管）。
- **PDF 纸宽填充（2026-09-13 用户定：应当填充）**：kookit PdfRender 渲染时刻定 canvas 像素、
  无重排入口 → **PDF 改纸宽时阅读器原地重开**（同布局模式的 `reopenTick` 机制，位置自动恢复）。
...
- **显示设计**：**参数项全部在挂载线右段下方**（字号/行距/段距/纸宽/内边距/佈局），**默认展开**
  ...
  布局模式 2026-09-13 从设置页移入面板（滾動/單頁/雙頁，改 = 重开书生效；`readerSettings` 唯一写者 = 面板）。
  分工依据 = `STYLE.md` §5.9 的频率表。
- **右抽屉两格：閱讀參數 / 聊天（2026-09-17 用户定，v0.4.3）**：右段与左段（目錄 / 筆記）**同构** ——
  同一几何、顶部两格文字开关切换（`.rail-tabs`，两侧共用同一对类名）。
  **聊天室只在"经房间进入的那本书"上存在**（用户："沒有通過房間進入一本書就不會有這個項"；
  判据 = 当前房间会话绑定的 edition === 阅读器打开的书）；不在房间里时右段只有参數一种内容、
  **连页签行都不渲染**（与 v1.10 之前的形态一致）。
  - **聊天的生命周期归属于房间**：进房才有、离房即消失（历史仍在服务器，随房间级联清理）；
  - 消息**与「房间」功能组件的会话视图是同一份**（渲染层共享态 `features/roomSession.ts`，
    经 `IRoomSession` 的 `chat-history` / `chat-message` 投影，按 id 合并去重）——不各持一份；
  - 发送走 `IRoomSession.sendChat`（server 落库后广播 `room.message` 回执含发送者）；
  - 视觉与交互（消息行、遮罩按距离、输入区、自动滚到底）见 `STYLE.md` §5.8；快捷键 `c`。

**目录（挂载线 + 垂挂列表；2026-09-12 并入挂载线实体）**
- **挂载线**：一条**横向细线，自 `--sidebar-w` 延伸整个页面宽度** = 目录（左段垂挂）与阅读参数
  （右段垂挂）共同挂载点；线是**固定的一条**（长度/位置只取决于侧边栏与窗口宽，**与纸宽无关**）；
  **目录折叠时点线（左段）= 展开目录**。
- **左右镜像（2026-09-16 用户定；v1.9 纠正口径）**：两段**关于「挂载线的中心」镜像** ——
  线 = 固定一条，两段**等宽**（`--rail-panel-w`）且**各贴线的一端**（等宽 + 贴齐 ⟺ 互为镜像，
  没有中心点算式）；**同高** = 共用 `--rail-panel-h` 上限。⚠ **宽度不许写成纸宽的函数**
  （v1.8 那样写会被判错：改纸宽会带着两段一起变）。
- **纸给挂件让位（v1.10 用户定）**：纸宽取 `--page-w = min(用户档位, 视口 - 2×(侧边栏 + 挂件 + 缝))`
  —— **让位的是纸，不是线**（用户："会盖住页面的内容…想办法解决"）。让位量必须含 `--sidebar-w`
  （纸窗口居中、挂件从侧边栏右缘起算）；输入栏的宽度也读同一个 `--page-w`。
- **左抽屉顶部的两格页签（v1.10 用户定）**：「目錄 / 筆記」**等分抽屉宽度、文字各自居中**，
  即**关于抽屉的几何中心对称**（不加 gap，中点落在抽屉中心上）。
- **笔记行（v1.10 用户定）**：行内「**編輯**」（静息隐形、hover 显形）打开底部输入栏并带出该条正文，
  **焦点自动落在输入区**；`body` 非空的行在元行带汉字符号「**註**」——用来**区分批注与高亮**。
- **跟随当前位置（2026-09-16 用户定）**：**用户读到哪里，哪个条目就落在列表正中** ——
  目录按"章号 ≤ 当前位置的最后一条"，笔记按"当前章的第一条"（本章无笔记则退化为之前最后一条）；
  判据在 `components/readerFollow.ts`（纯函数 + 单测），打开/换书瞬时落位、跨章平滑跟随。
  ⚠ 位置只到**章**（kookit 不给章内进度），章内精确到某一条见 `TODO.md`。
- **点高亮 = 看内容（2026-09-16 用户纠正）**：单击正文高亮**不再开编辑器**（用户："我显然是希望看
  详细内容或者其他原因，而不是为了更改"）→ 左侧切到「筆記」并把该条定位到正中（只读）；
  **编辑仍走右键菜单的「編輯批註」**。
...
- **空目录占位**（2026-09-13 用户定）：**没有目录索引的书也展开垂挂区**，显示「本書沒有目錄索引」
...
- **右侧"模型阅读"容器**（预留）：右侧已有阅读参数面板，将来若也要右侧**先定共存形态**；
  原则不变：**只占留白，不挤压纸的语义**。
- **同步相关控件** → **已定形态（2026-09-17，v0.4.3）**：阅读页的同步入口 = **挂载线右段的第二格
  「聊天」**（见上「右抽屉两格」），成员列表/离开房间仍留在「房间」功能组件的会话视图里；
  位置上报（跟随模式等）的其他同步控件仍未定，随 `TODO.md`「location-updated 同位 UI（跟随模式）」一起做。

## 12. 笔记管理工具组件（**已实施 2026-09-16**）

> 动机（用户原话）："当用户的笔记积累到近千条，管理和索引也就变得同样重要了"。
> 入口形态 = **左侧工具组件**（用户 2026-09-15 定；**不再考虑**"书库侧栏第三个入口 / 书库页下的视图"）。
> 本节定**形态与容器契约**；**视觉语汇在 `STYLE.md` §5.10 立案**；实施步骤见根 `../../TODO.md`。
> 契约：`CONTRACTS.md` **v0.4.1**（§4.4 读模型）+ **v0.4.2**（`NoteView`/`NoteFilter`/`NoteTextFocus`/`NoteSettings`/`IClipboard`）
> + `DATA_MODEL.md` §5 问题 6（检索批复）。
...
### 12.3 视图、内容与筛选（**视觉语汇已在 `STYLE.md` §5.10 立案**）

- **视图两态 = `網格` / `瀑布流`（默认瀑布流），没有列表形态**（用户 2026-09-16 定）。
  列宽固定、**只变高**；高度由内容自然决定但两头有界（摘录 ≤4 行、批注 ≤6 行）。细则见 `STYLE.md` §5.10。
- **条目标签 = 「書名 · 節 N」**（`editionTitle` + `anchor.norm.chapterIndex + 1`）+ 标记块
  + 摘录/批注 + 相对时间。
  - **章标题 v1 不做**（`notes` 不存章标题；跨书列表里**书名**才是有用的标签）—— 理由与将来做法见 `DATA_MODEL.md` §5 问题 6。
- **筛选（底部状态栏，单按钮三态循环）**：`批註 → 劃線 → 全部`，**默认「批註」**。
  ⚠ **判据 = `body` 是否为空，不是 `kind`**（两者在实现里可互相矛盾，见 `CONTRACTS.md` §2 `NoteFilter`）。
- **文字主次（单按钮两态循环）**：`批註為主`（默认）/ `摘錄為主`；筛选为「劃線」时禁用。
- **作用域（单按钮两态循环）**：`當前庫`（默认）/ `全部庫`。
- 颜色 / 有无批注 / 关键词仍可作为读模型的筛选维度（`NoteQuery`）；排序默认 **`updated` 倒序**。
- **平铺、不按书分组**：分组头会打断瀑布流的流向，且每条已带书名（信息冗余）。
- **计数显示筛选后的条数** —— ⚠ 与书库"不显示书目数量"不同：这是**筛选器的必要反馈**。
- **空态（缺失要可见，不静默）**：当前筛选为「批註」时**必须报出被隐藏的划线数量**并提示切换。
- **检索入口 = 标题栏搜索栏 · 笔记作用域**（用户 2026-09-16 定；见 §12.8）。
- **待定（实施期定，不阻塞）**：批量操作（清理 / 导出？）。
- 近千条性能：**已实现窗口化**（自实现装箱 + 按列二分，见 `STYLE.md` §5.10 的 A/B 与两个实现期坑）；
  实测 1000 条时 DOM 只留 **42** 张卡片、首屏 22ms、整片重渲染 21ms。

### 12.4 跳转链路（本次**唯一新增的机制** —— 补上一个契约缺口）

- **鼠标分工：单击 = 选中、双击 = 跳转**（与书库资源管理器语义一致；用户 2026-09-16 定）。
  卡片自身已展示内容，**不做详情抽屉**。
...
- **卡片右键 = 挂载线菜单**（复用 `ContextMenu` 形态；**语义按域** —— 用户 2026-09-16 明确
  "右键在不同功能组件里语义不同"）。项：
  - `跳轉` —— 等同双击；
  - `複製批註` —— **仅 `body` 非空时出现**；**只複製批註正文**（用户 2026-09-16 定），
    走新增端口 `IClipboard.writeText`（`CONTRACTS.md` §4.6）；
  - `刪除` —— 见下。
- **卡片内不设 hover 动作区**（用户 2026-09-16 定：卡片信息量不比书籍，右侧动作列取消 → 卡片保持零 chrome）；
  也不放开拖选（**守 `STYLE.md` v1.1 全局禁选**：文字选择 = 阅读正文专属），复制走上条按钮。
- ⚠ **用词纪律（用户 2026-09-16 定）**：书库的「**移除**」= 只删索引、**不删源文件**；
  笔记的删除是**真实删除** → 一律写「**刪除**」，**两词不得混用**。
  确认弹窗写明**不可恢复**，且**不提供"下次不再提示"**（不可逆动作不能免确认）。
- 键盘（方向键移焦点 / Enter 跳转 / Delete 刪除）随**意图层**收编（`TODO.md` 高优项），本专题只留接口。

### 12.7 视觉（**已实施** = `STYLE.md` §5.10）

- 渲染层红线（`MAP.md` 红线 / `STYLE.md`）：**新组件不得自创视觉语汇** → 已按此在 **`STYLE.md` §5.10** 立案
  （卡片三段结构、两档文字主次、选中态试验档、状态栏 5 件、空态口径、`--note-col-w` 登记），
  并连带修订 §5.1（三角负片扩为"视图切换"这个功能共用）与 §5.2 例外⑤（改写为"色块 = 功能信息"）。
- ⚠ **选中态是试验档**：1px `--accent-ring` 直角矩形 + 卡片常驻 8px 内边距；按 `STYLE.md` §7 纪律
  **单 commit 可回退**，实测若与 P1 冲突 → 回退为"元行套负片块"。

### 12.8 检索接线（标题栏 · 笔记作用域）

- 与 2026-09-12 已定的"标题栏搜索 = 全局、**按功能域分作用域**"一致：笔记管理的作用域搜**笔记**
  （`excerpt` + `body` 子串），`Ctrl+F` 聚焦 / `Esc` 清空复用。
- ⚠ **实现注意**：`TitleBar` 现在只把 `value/onChange` 接到书库，且输入框带 `key={activeFeature}` **按功能重挂**
  → 须扩为**按 `FeatureId` 分表的搜索词**（`AppShell` 持有），否则笔记域的输入会与书库的词互相串扰；
  `app.focusSearch` 意图的分流也要从"仅书库"扩到"书库 / 笔记"。
- ⚠ 与另外两类检索**不是一回事**（见 `TODO.md`「全局搜索接线」）：阅读器作用域 = `IRenderService.search`（书内全文）；
  "跨书库全文搜索"**没有待办**，属新专题。
