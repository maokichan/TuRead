...
- **导入菜单**：「导入文件…」/「导入文件夹…」（⚠ Electron 不允许文件与目录同框选择，[issue #26885](https://github.com/electron/electron/issues/26885)）；
  菜单首项是**「文件夹含子目录」复选框**（持久化 `librarySettings.importRecursive`）—— 决定导入"仅此节点"还是"此节点及所有子节点"；
...
- **抽屉位置纪律**（2026-09-08 定）：**状态栏之上是内容区，抽屉只在内容区弹出** —— 抽屉由内容区容器
  `relative` 定位为 `absolute inset-y-0 right-0`，**不覆盖底部状态栏**；被它盖住的内容不显示也不可点。
...
- **无封面 → 文字封面**：`FittedTitle` 二分搜索**不溢出的最大字号**（ResizeObserver 跟随尺寸），
  **加粗**、行高 1.02、字距略松，中文用**源流明体字栈**（`--font-serif-cn`，未安装回退系统宋体）——
  目标是"留白最少、用字体制造冲击"的平面设计手法。列表模式同理：无封面时左栏用文字封面**且不套渐隐遮罩**
  （否则标题会被淡出吃掉），有封面时才是"左侧填充 + 向右渐隐"。
- **主题色取向**（2026-09-08 定）：**暗色 / 亮色两套一律黑灰白**，不引入色相（此前的蓝色 accent 与偏蓝底色已清除）；
  高亮色 = 灰阶两端（暗色主题近白、亮色主题近黑）；**羊皮纸**保持暖棕、其高亮用**深棕**；
  状态色（ok/warn/err）保留语义色相。
