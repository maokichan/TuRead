...
- **显示设计**：**高频项挂在阅读页左侧挂载线底部**（字号/行距/段距/纸宽/内边距，把手开合、
  面板**向上展开**、内容中间对齐，`p` 开合；2026-09-12 并入挂载线实体，原右侧召唤条废除）；
  **低频项在「设置」**（布局模式）。分工依据 = `STYLE.md` §5.9 的频率表。

**目录（挂载线 + 垂挂列表；2026-09-12 并入挂载线实体）**
- **挂载线**：一条**贯穿全页高的细竖线** = 目录（顶部垂挂）与阅读参数（底部挂载）共同挂载点；
  **左缘 = 侧边栏展开时的右缘**（`--sidebar-w`），
