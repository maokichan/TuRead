...
- **挂载线**：一条**横向细线，自目录段延伸整个页面宽度** = 目录（左段垂挂）与阅读参数
  （右段垂挂）共同挂载点；**线的左端 = 侧边栏展开时的右缘**（`--sidebar-w`）；
  **左右段宽 = 两挂件的宽度（两侧相同）**→ 默认不侵入正文列；
  **目录折叠时点线（左段）= 展开目录**。
- **左右镜像（2026-09-16 用户定）**：两个挂件**以页面中线镜像** —— **同宽、同高、离中线等距**
  （`--rail-panel-w` / `--rail-panel-h` / `--rail-gap`；算式见 `STYLE.md` §5.8）。原"左挂件贴侧边栏、
  右挂件贴窗口右缘"的各自夹取已退役（那两个 token 不再存在）。
