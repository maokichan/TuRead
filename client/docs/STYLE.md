...
（`.view-switch::after`，取 `currentColor`）。理由：它的职责是**引导用户点击切换视图**，
需要额外的可点提示；其他文字按钮不需要。新增带三角的按钮必须先改本文。
...
| 偏差 | 位置 | 处置 |
|---|---|---|
| 设置页仍用 13px 无衬线边框分段按钮 | `SettingsFeature/index.tsx:113-115` | 按 §5.1 改文字档（下一步） |
| 列表标题 17px 用 `font-normal`，与"衬线只用 700"冲突 | `BookRow.tsx:83` | 保留视觉（700 在小字号偏重）；记为**唯一豁免**，其他衬线一律 700 |
| 房间页的输入框/表单仍是无衬线 13px | `RoomFeature/index.tsx:16-22` | 属 §5.2 例外①，**保留**（输入类必须可见可点） |
...
- 抽屉外壳**零 chrome**：无边框、无阴影，底色取 `--bg`（视觉上"透明"，但必须遮挡下层列表——
  真透明会让两层文字叠在一起）。
