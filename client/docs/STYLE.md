...
- 外壳**完全透明**：无边框、无阴影、**无底色** —— 下层书库内容从文字块之间透出来；
  可读性由负片块自身保证。
- 结构（2026-09-09 用户定）：封面 + 标题 → **滚动信息行**（格式 · 文件大小 · 导入时间，
  `.drawer-ticker`，16s 循环、hover 暂停、`prefers-reduced-motion` 下不动）→ 字段块
  （描述占位 / 阅读进度 / 最近阅读（**相对时间**）/ 文件路径）。
  已移除：当前位置、指纹（没必要）、文件大小与导入时间（并入滚动行）。
- 抽屉内的动作按钮用 `.text-action`（与书库底部状态栏同一套）。
