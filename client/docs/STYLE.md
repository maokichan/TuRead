...
```
┌──────────┬──────────────────────────────────┐
│          │ 标题条（负片，过长自己滚）         │  ← 三行左对齐「封面右缘」，
│  封面     │ 数据行（格式 · 大小 · 导入时间）    │     撑满抽屉宽度
│ 64×96    │ 指标条（进度｜最近｜总时长）        │
├──────────┴──────────────────────────────────┤
│ 描述块（满宽负片）                            │  ← 更多信息：满宽、左边距对齐封面左缘
│ 文件路径块（满宽负片，可换行）                 │
└─────────────────────────────────────────────┘
```

- **三行必须单行**：标题与数据行都用 `Marquee`（`components/Marquee.tsx`）——
  先量宽度，**超出才滚**（无缝 = 两份内容 + 各自 `padding-right`，位移 -50%）；
  hover 暂停；`prefers-reduced-motion` 下静止。**绝不换行向下生长**。
- 行 3 = **三个独立容器**（等宽、标签 11px / 值 12.5px、单行截断）：阅读进度 / 最近阅读（相对时间）/ 总阅读时间（占位）。
- 外壳**完全透明**（无边框、无阴影、无底色），下层书库内容从文字块之间透出来。
- 动作按钮用 `.text-action`（与书库底部状态栏同一套）。
- 已移除：当前位置、指纹（没必要）；文件大小与导入时间（并入数据行）。
