...
`11 · 12.5 · 13 · 14 · 15 · 17 · 18 · 19 · 20`，以及封面自适应 `10–120`（`FittedTitle` 二分拟合）。
`20` 仅用于书库底部状态栏的两个文字按钮（`.text-action--lg` / `.view-switch`）。
...
两个文字按钮放大到 20px（`.text-action--lg` / `.view-switch`）。
...
**唯一用途**：书库详情抽屉（`BookDetailPanel`）的标题与字段。
- 每个文字块 `inline-block max-w-full bg-[var(--negative-bg)] text-[var(--negative-text)]`，
  **左对齐**，块宽随内容（长值换行、`break-all`）。
- 抽屉外壳**完全透明**：无边框、无阴影、**无底色**（2026-09-09 用户定）——
  下层书库内容从文字块之间透出来；可读性由负片块自身保证。
- 字段清单（2026-09-09 用户定）：**移除「当前位置」「指纹」**；保留「描述」占位（内容待定，见 `TODO.md`）、
  文件大小、阅读进度、导入时间、最近阅读、文件路径。
- 抽屉内的动作按钮用 `.text-action`（与书库底部状态栏同一套），不再有边框按钮。
