...
**server v0.2.0 已实现；client v0.1.15（2026-09-13 已发版，见下）。**
...
- **client v0.1.15（2026-09-13，已发版）—— 首个打包发行版 + 书架三则**：
  把此前 v0.1.13/v0.1.14 只提交未打包的内容一并交付；本轮新增**全局禁选**（文字选择 = 阅读正文专属，
  宿主 UI 一律不响应拖选，输入类豁免）、**书库层级后退/前进**（历史栈 + 标题栏按钮 + 鼠标侧键，
  对齐文件资源管理器）、**面包屑根固定**（固定宽度容器，根锚死、子节点向右增生）。
  验证：typecheck 双绿 + `library` 探针 20 断言全过；**真机验收通过（用户 2026-09-14：全局禁选 ✓ / 鼠标侧键 ✓）**。
  发行物 = **免安装便携版** `release/TuRead-0.1.15-win-x64-portable.exe`（不出 NSIS 安装包，用户 2026-09-14 定）。
  **交接快照见 `docs/STATUS.md` §6**（下一步主线与验证工具链）
- **client v0.1.14（2026-09-13）—— 沉浸全屏 + 交互根修 + 数据地基**：
  ...
  **书库双模式**（建库二选一：虛擬映射=唯一根文件夹递归导入/自建書箱；资源管理器式层级浏览，
  書箱=一等条目：单击选中/双击进入/F2更名/右键菜单/拖书入箱=移动，右下角当前层级面包屑，
  拖拽文件进窗=导入当前库）；**实机验证通道**（`TUREAD_USER_DATA` 独立 userData + library 探针 +
  computer-use 真实点击）；**定位转换机制**设计提案（DATA_MODEL §3.1.1）；
  适配器导航"静默 no-op"销案（未打开即抛错）；Esc 分流纠正（不收参数面板）；
  **交接快照见 `docs/STATUS.md` §6**（下一步主线与验证工具链）
