...
**server v0.2.0 已实现；client v0.1.14（2026-09-13，见下）。**
...
- **client v0.1.14（2026-09-13）—— 沉浸全屏 + 交互根修 + 数据地基**：
  **沉浸全屏**（F11 + 设置开关，标题栏动画收起，Esc 分流：面板→全屏→退阅读器，离开阅读器自动还原）；
  **iframe 事件桥**（键盘/滚轮被文档边界挡住的根修，递归嵌套覆盖 PDF 子页，按键只看界面不看焦点）；
  **分页模式滚轮翻页**（无按钮控件，累积+冷却）；**参数面板默认展开 + 布局模式进面板**（重开书生效）；
  **空目录占位**（「本書沒有目錄索引」）；**PDF 改纸宽原地重开填充**；挂载线 28→44px；
  **SQLite 单库已落地**（SqliteStore 替代 JsonStore + one-shot 迁移器，library.json/config.json
  改名 .migrated 留档；`TUREAD_USER_DATA` 支持独立 userData 验证）；**多书库落地**
  （config.json=引导文件只存库注册表，一库一 .db，底部状态栏「書庫」菜单切换/新建）；
  **定位转换机制**设计提案（DATA_MODEL §3.1.1）；适配器导航"静默 no-op"销案（未打开即抛错）；
  Esc 分流纠正（不收参数面板）
