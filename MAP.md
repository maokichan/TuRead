...
- **本轮（2026-09-16 之二，已提交未发版）「阅读器跟随与镜像」**（用户四条 + 一条修 + v1.9 纠正）：
  目录/笔记的**当前条目跟随阅读位置落在列表正中**；**挂载线两段关于「线的中心」镜像**（线是固定的一条，
  两段等宽且各贴线的一端 —— ⚠ **宽度不许由纸宽派生**，v1.8 那样写过、被用户驳回）；
  **右键挂载菜单提高对比度**；**批注输入栏 = 底部居中的纯色带边框零文字容器**（单行高、宽 = 纸宽 × 0.9，
  **生命周期绑在它的对象上**：选区型随选区消失、笔记型随笔记消失）；**单击正文高亮 = 看内容（左侧定位）而非改内容**；
  另修**笔记排序**（按阅读先后）+ **右键菜单坐标从未换算成宿主坐标**（真 bug）。
  判据 = `STYLE.md` §5.8/§5.2 例外⑥ + §10 **v1.9**；验证 = typecheck 四 project、`npm test` 98、
  样张 `smoke.cjs` 四条几何机检全绿、`note` 探针全 PASS（`library` / 渲染自检亦无回归）。
