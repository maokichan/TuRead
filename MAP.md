...
- **「书的身份」已落地并打 tag `client-v0.1.17`**（**未打包**）：**书库降为组织模式**（全局单库，不再是"一库一 .db"）；
  ...
  **映射库的"移除"只移除可见性**（不能在其中加书，靠**扫描**对账）。
  ...
  ⚠ **真机验收未做**。
- **笔记管理「工具组件」契约已定（2026-09-16），代码未动**：**「工具组件」就是统一容器的「功能组件」**（同级、同容器，
  **不引入 `kind` 分类**）；作用域默认当前库、可切全部庫；检索**只搜笔记**（v1 `LIKE`，FTS5 不上）；
  列表标签「書名 · 節 N」。跨书跳转补上通道 `openReader(editionId, {revealNoteId})`。
  契约 = `FEATURES.md` §12 + `CONTRACTS.md` v0.4.1 + `DATA_MODEL.md` §5 问题 6；**实施步骤见 `TODO.md`**。
