...
- **「书的身份」已定案并落地**（未发版）：**书库降为组织模式**（全局单库，不再是"一库一 .db"）；
  **笔记挂 edition**（跨库共享、各版一份）；**阅读时间逐 edition 记录、按 work 汇总**；
  **映射库的"移除"只移除可见性**（不能在其中加书，靠**扫描**对账）。
  契约 = `client/docs/DATA_MODEL.md` §4.2/§6 + `CONTRACTS.md` v0.4.0；步骤与验证见 `TODO.md` 文首 ★ 块。
- **插件态度**：v1 不做插件运行时，**ports 即插件边界**
  （官方插件 = 注册进 ServiceContainer 的适配器 + 追加 FeatureDescriptor 进 registry）
- **两个专题待做**（用户 2026-09-15 定，入口形态已定）：**笔记管理 → 左侧工具组件**；
  **阅读时间 → 自己的弹窗与界面**。见 `TODO.md`。
- 下一步见 `TODO.md`（唯一待办清单）
