...
| # | 行动 | 归属 | 已登记 |
|---|---|---|---|
| 1 | 排版参数：剩余参数照常落挂载线面板/设置页；增加"单书覆盖全局" | 本地侧 | `TODO.md` 渲染与阅读器 |
...
1. **阅读器不管理源文件（实体层让位）**。对书籍**源文件实体**（拷贝、整理、资产管理）阅读器不做——
   导入 = 建索引不拷贝、移除 = 只删索引（现状即如此），实体交给用户自己已有的工具
   （用户本人现状的例子是 TagHit 这类专门的资产管理程序）。⚠ 这**不是**"要求用户装某个特定程序"，
   也**不延伸到索引组织层**：**容器/书架系统照常演进**（2026-09-09 群聊已定案，用户总要用一种方式索引信息），
   TuRead 也不依赖、不绑定任何外部程序。
   **佐证**：Calibre 的"重度管理+阅读"混合是它被嫌弃笨重的根源；本判断是对既有做法（索引不碰源文件）的确认。
2. **参数控制有必要，但形态克制**。参数是刚需（用户二次澄清：**参数控制也是有必要的**），
   要拒绝的是 Koodo 式**形态**——控制参数常驻铺开、排版参数过分占据注意力（KOReader
   "overwhelmed by all the options" 的社区共识差评，§2.1）。形态纪律：召唤式、默认收起、
   默认暴露集最小、改动即时可见——这正是召唤式面板（v0.8 起，现挂载线右段）已在做的，继续坚持即可。
...
- 判断 1 只覆盖**源文件实体层**，与书库索引组织（容器/书架）无关——后者已由 2026-09-09 群聊定界、
  照常演进（见 TODO「多书架/书库容器」）。
...
2. **⚠ 初稿过度外推的记录（2026-09-12 二次澄清）**：本节初稿曾把判断 1 外推为"阅读器=索引，不自建容器/书架、
   冻结 TODO 容器项、远期靠外部程序互通"，并把「剩余阅读参数」降速——**用户已纠正**：容器与书架系统是
   **已定案要做的**（用户总要用一种方式索引信息），参数控制也是**有必要的**；判断 1 只约束"不管理源文件"，
   判断 2 只约束参数的**形态**（克制、召唤式）而非其存在。特此记录，防止后续会话再次过度外推。

### 6.4 判断的落点（哪些东西变了）

| 影响面 | 变化 |
|---|---|
| TODO「多书架/书库容器」 | **照常演进**（2026-09-12 用户确认）：判断①只约束源文件层，不延伸到索引组织 |
| TODO「剩余阅读参数」 | **照常评估落地**（2026-09-12 用户确认参数控制有必要）；形态纪律 = 召唤式、默认收起、默认暴露集最小 |
