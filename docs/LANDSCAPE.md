...
1. **阅读器 = 索引，不做实体管理**。桌面阅读器的功能应该只有"索引"（对书籍的轻量引用与阅读状态），
   图书实体的组织/管理（分类、标签、集合、资产管理）不该交给阅读器，而应交给专业的数字资产管理程序
   （用户自己的 TagHit：标签替代层级、跨目录、数据本地，见 `D:\PROJECT\TagHit\README.md`）。
...
2. **索引边界 vs 即将做的"容器"**：TODO 的「多书架/书库容器」方向（分类树/书单/tag 系统）在判断 1 下
   **属于 TagHit 的职责**——TuRead 若做容器，就在重走 Calibre 的路。该项已标注冻结重审（见 TODO.md）；
   将来的正确形态更可能是"TuRead 索引 ← TagHit 组织"的**互通**（如 TuRead 订阅 TagHit 的工作区/标签作为
   书籍过滤器），而非自建容器模型。
...
| 影响面 | 变化 |
|---|---|
| TODO「多书架/书库容器」 | **冻结重审**：分类/容器/tag 归 TagHit；TuRead 只留索引，远期考虑 TagHit 互通端口 |
