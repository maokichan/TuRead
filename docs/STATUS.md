...
| **数据分层存储（2026-09-12 用户批复）** | **两层并存**：① **书库/阅读状态 = 单一 JSON 文件**（现状 JsonStore 保持）——一份文件对应一份书库，用户可直接管理/携带自己的数据，跨平台靠这份数据文件保存阅读状态，避免"不同方式打开后内部数据不一致"；② **笔记/标注（预期上万条）= SQLite 严肃考虑**（笔记功能落地时立项）：高频小写 + 查询是 JSON 全量重写扛不住的。⚠ 远期几乎**所有数据都要在服务器同步**（房间共读产品的前提），本地存储选型以"易于整体同步"为约束 | `docs/STATUS.md` §3；`TODO.md` 工程组 |
