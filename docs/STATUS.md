...
> 更新：2026-09-16（**之二 · 未发版**）（**阅读器「跟随与镜像」** —— 用户四条：目录/笔记的**当前条目跟随
> 阅读位置落在列表正中**、**挂载线下两挂件以页面中线镜像**（同宽/同高/等距）且条目放大、
> **右键挂载菜单提高对比度**、**批注输入栏改为底部居中的纯色带边框零文字容器**；另修**笔记排序**：
> 按阅读先后而非创建时间。判据 = `STYLE.md` §5.8 + §5.2 例外⑥；验证 = typecheck 四 project、
> `npm test` **98**、样张 `smoke.cjs` **新增三条几何机检全绿**、note/library/渲染自检三个真机探针无回归。
> **PDF 翻页**已按用户定调**只登记为专题**（`TODO.md` PDF 专区），本轮不动手）
...
| **阅读器跟随与镜像（2026-09-16 用户定；v1.9 纠正几何口径；v1.10 补五条）** | 用户四条：① **目录/笔记的当前条目跟着阅读位置走，落在列表正中**（"我们读到哪里，哪个条目就处于目录的中间位置"，"这一点同样反应在笔记上"）② **挂载线下两挂件关于「挂载线的中心」镜像**（同宽、同高）③ **右键挂载菜单提高对比度**（用户："点击创建笔记的右键挂载整个容器太透明了"）④ **批注输入栏改形态**：底部居中、纯色带边框、**零文字**（无标题/提示词/按钮），操作逻辑不变。另：**笔记按阅读先后排序而非创建时间**（用户定）—— 排序权威上收领域层 `compareNoteOrder`（存储与 UI 共用一份）。⚠ **v1.9 纠正（用户驳回 v1.8）**：v1.8 把两段宽/位置写成**纸宽的函数**（"整个挂载线变成了相对于中间显示内容的绝对长度，这点更是可笑之极，谁让你改的？"）→ 改回**线是固定的一条**、两段**等宽且各贴线的一端**（等宽+贴齐 ⟺ 互为镜像，无中心点算式）；批注输入栏改为**纸宽×0.9**，并把**生命周期绑到它的对象**（选区型随选区消失而关；**笔记型随那条笔记从库里消失而关**）；**单击正文高亮 = 看内容（左侧定位）而非改内容**。⚠ **v1.10 补五条（用户第 4 轮）**：㈠ **纸给挂件让位**（"会盖住页面的内容"）→ `--page-w` 唯一定义处 = 用户档位 ∩ 视口−2×(侧边栏+挂件+缝)；㈡ **左抽屉两格页签关于抽屉的几何中心对称**；㈢ **输入栏自增长**（空态两行 → 五行封顶转滚动，`field-sizing: content`）；㈣ **笔记行补「編輯」**（打开输入栏 + 焦点落输入区）**与「註」符号**（`body` 非空 → 区分批注/高亮）；㈤ **退出阅读器保持最大化**（进入阅读器时是 F11 → 离开时退出全屏 + `win:set-maximized`）。判据 = `STYLE.md` §5.8/§5.2 例外⑥ + §10 v1.8~v1.10；实现 = `components/readerFollow.ts`（纯函数 + 单测）、`ReaderRail`/`TocPanel`/`NotesPanel`/`NoteComposer`/`styles.css`、`shared/ipc.ts` + `main`（`win:set-maximized`） | `client/docs/STYLE.md` §5.8 + §10 v1.10；`FEATURES.md` §11；`client/src/renderer/src/components/readerFollow.ts` |
| **右键菜单坐标（v1.9 修的真 bug）** | 适配器 `onContextMenu` 把**书文档里的 `clientX/clientY`** 当宿主坐标发了出去（注释写着"换算"，代码没换）→ 正文里选完内容右键，菜单出现在**非内容处**（用户："非常可笑"）。修法 = `hostOffset()` 沿 `frameElement` 链**逐层累加**（PDF 每页是子 iframe，不止一层），`selectionRect` / `onNoteClick` 同批改用它。⚠ **旧探针把 bug 断言成了契约**（`ctx.x === 240` = 原始坐标），已换成"换算后的宿主坐标 + **必须落在纸内**" —— **探针断言本身也要接受怀疑** | `client/src/core/adapters/render/kookitRenderAdapter.ts`；`client/src/renderer/src/dev/noteProbe.ts` |
| **PDF 翻页 = 独立专题（2026-09-16 用户定，登记未做）** | 用户："其实直到现在，翻页仍然存在大量的问题"→ 归因**主要是 PDF**，"操作是不符合逻辑的"，期望形态 = **浏览器自带 PDF 阅读器 / Koodo Reader 那样**；并明确"**我们以后会作为一个专题去聊**" → **本轮只登记不动手**。既有可复用事实（探针绕开了真实鼠标路径、点击带 1/4 与 STYLE 的 1/3 口径不一致、PDF 改纸宽走原地重开）已抄进 `TODO.md` PDF 专区新条目 | `TODO.md` PDF 专区「PDF 翻页交互重做（专题）」 |
...
`client/package.json` version = **0.1.18**；**本轮「阅读器跟随与镜像」已提交、版本号未滚**
（按 §2：提交与 tag 由 agent 执行、版本号由用户决定 → 该批改动并入 v0.1.18 之后的当前版本，
**待用户指示发版时再滚**）。发行物仍是上一版的免安装便携版
...
**★ 现在在哪（2026-09-16 之二 / **v1.10**）· 阅读器「跟随与镜像」（未发版，真机验收待用户走）**
- 权威：**`STYLE.md` §5.8（「挂载线」「左右镜像」「纸给挂件让位」「左抽屉两格页签」「跟随当前位置」
  「右键挂载菜单」「批注输入栏」「左抽屉的笔记行」八行）+ §5.2 例外⑥ + §10 **v1.8~v1.10****；
  功能描述 = `FEATURES.md` §11；决策汇总 = §3 的「阅读器跟随与镜像」行 +「右键菜单坐标」行；
  未做项 = `TODO.md`（**笔记内容导出**、**划线样式 + 批注/高亮区分**、PDF 翻页专题、章内精确化、
  当前条目无视觉标记、点高亮的"看内容"形态）。
- ⚠ **v1.10 这一轮我自己踩的两个坑（同一类：同一算式的两处落点）**：㈠ `--page-w` 的让位量漏了
  `--sidebar-w`（纸**窗口居中**、挂件**从侧边栏右缘起算**）→ 实测仍压纸 40px；
  ㈡ 输入栏独立算了一次 `min(100%, --read-width)*0.9` → 比被夹窄的纸还宽。
  → 教训：**"生效值"必须只有一处定义**（现 = `:root --page-w`，纸与输入栏都读它）。
- 实现落点：`components/readerFollow.ts`（判据纯函数 + 单测）｜`ReaderRail`（两段等宽贴线两端、
  `activeChapter` / `focusNote` 透传）｜`TocPanel` / `NotesPanel`（跟随 + 「看这条」定位 +
  行内「編輯」/「註」）｜`NoteComposer`（底部居中、零文字、自增长 2→5 行）｜`styles.css`
  （`--page-w` / `--rail-panel-w` / `--rail-panel-h` / `--rail-gap`、页签对称、菜单与输入栏对比度）｜
  `domain/anchor.ts` 的 `compareNoteOrder`（排序权威）｜适配器 `hostOffset()`（宿主坐标换算）｜
  `shared/ipc.ts` + `main` 的 **`win:set-maximized`**（退出阅读器时"全屏 → 最大化"）。
- 验证（v1.9+v1.10 实测）：`typecheck:all` 四 project 全绿；`npm test` **98**；样张 `smoke.cjs`
  **六条几何机检全绿**（`mirrorOk` 180/180 同宽贴齐；`widthIndependentOk` 改纸宽 260→520 宽度不变；
  `paperClearOk` 纸与挂件不相交；`tabsSymOk` 两格中点 = 抽屉中心 = 0 偏差；`followedOk` 偏差 1px；
  `composerOk` 宽偏差 0 / 空态 60px≈2.6 行 / 灌 8 行 130px≈5.6 行 / `fieldSizing=content`）；
  真机 note 探针 **全 PASS**（纸 x[352..848] 与挂件各留 16px、输入栏 446.4=496×0.9、空态 60.1px、
  封顶 129.9px 且 194>116 转滚动、「註」符号=1、抽屉「編輯」带出正文 + 焦点在输入区 + Esc 关闭、
  右键坐标落在纸内、选区消失→输入框消失、Enter 提交落库、重开后自动回挂）。
  ⚠ **三处"量法/分段/模板"教训**（都会造成假绿或假 FAIL）：① 描边宽度不能断言字面 `1px`
  （DPR 1.25 → 0.8px）；② 居中要跟 `documentElement.clientWidth` 比；③ **探针分段也是判据**
  （不依赖重开的断言不许挂在重开上）；④ 探针模板字符串里**不能写反引号**（会截断模板 → Electron
  弹错误框 → 表现为"卡死"；本轮踩了两次）；⑤ **受控组件在样张里必须自己持有 state**（否则
  `value="" + onChange={noop}` 会让"自增长"这类判据测不出来 —— 假绿）。
- ⏳ **本次没做**：① **PDF 翻页专题**（用户定调"以后作为一个专题去聊"→ 只登记）；② **笔记内容导出**
  与**划线样式/批注区分**（均为用户本轮新立项，形态待定 → 见 `TODO.md`）；③ 跟随居中的章内精确化；
  ④ 当前条目的视觉标记；⑤ 点高亮的"看内容"最终形态。
- ⚠ **待真机复看（探针证不了）**：**"退出阅读器保持最大化"**（窗口状态变化，无头探针量不到）；
  以及本轮所有手感类改动（输入栏自增长的手感、抽屉「編輯」的 hover 显形、页签对称的观感）。

**★ 上一轮（v0.1.18）· 笔记管理（跨书）已落地并真机验收通过**
- 权威：形态与交互 = `FEATURES.md` **§12**；视觉 + **窗口化 A/B 与两个实现期坑** = `STYLE.md` **§5.10**；
  契约 = `CONTRACTS.md` **v0.4.1/v0.4.2**；检索口径 = `DATA_MODEL.md` §5 问题 6；决策汇总 = §3 对应行。
...
- ⏳ **那轮没做、仍可做**（按建议优先级）：
  ...
  4. **发行物**：打一次便携版（v0.1.16~18 + 本轮内容）+ **许可文件随包**（对外发行前必修）；
  5. **旧账**：「重开书偶发空白」（已有可重复复现器）。
...
⚠ **2026-09-16 本轮复跑：多次连试两轮（每轮 3×15s）全灭**，与"约 1/4"的历史观测相比本次更稳定地复现；
**且出现过一次"半成功"**（新 iframe 出来了，但渲染侧未就绪：手工 `renderHighlighters` 被"当前节"过滤到 0 条、
左侧笔记面板空）—— 已抄进 `TODO.md` 作为本条的新证据。
⚠ **探针分段已重排（v1.9）**：只有「重开后自动回挂」一条依赖重开，命中时**只损失它自己**，不再整段跳过
（旧写法让 10 条断言陪着一起消失 —— **假绿与假 FAIL 一样危险**）。

**验证工具链（回归全靠它们）**：
- **`npm test`**（vitest）= **纯逻辑单测**（**98 断言**）：领域语义（anchor/location）+ **笔记阅读序**
  （`compareNoteOrder` 5 条，2026-09-16 加）+ **阅读器跟随判据**（`readerFollow` 7 条，2026-09-16 加）
  + **笔记流几何**（`noteLayout` 14 条）+ 分层依赖守卫 + 迁移/双队列。秒级、无需书。
...
- **`npm run typecheck:all`** = 四个 tsconfig（node / web / test / **preview**）。
  ...
  就是漏跑它造成的（15 处漂移它一个不漏）。⚠ 另：`tsconfig.test.json` **不设 `jsx`** ——
  纯 `.ts` 模块**不能** import `.tsx` 的类型（会 `TS6142`），数据形状要住在纯模块里（`TocRow` 因此搬家）。
- **样式样张探针**：`npm run style` 起样张，另开终端
  `node_modules\electron\dist\electron.exe tools\style-gallery\smoke.cjs [url] [--scale=N] [--no-window]`
  → 判据 = 有面板 + 有文字 + **无页面错误** + 笔记卡片高度多档 + **2026-09-16 新增三条几何机检**
  （`mirrorOk` 两挂件镜像 / `followedOk` 当前条目居中 / `composerOk` 输入栏零文字）；`--scale` 另量窗口化规模，
...
- `TUREAD_DEV_PROBE=library` + `TUREAD_DEV_BOOK=<书>` = 多库/書箱/笔记/**笔记读模型**探针
  （文档旧记 **56 断言**；**2026-09-16 本轮实测 50 行 PASS、无 FAIL** —— 口径未复核，先登记为观察；
  读模型那批含"`kind=highlight` 但补过 body 的必须算批注"、LIKE 通配符转义、经 `holdings` 的库作用域）。
