...
- **移动全量交互（本轮⑭）**：书与書箱均可拖拽（拖入箱、拖出箱到根段、拖到面包屑段）；
...
- `TUREAD_DEV_PROBE=library` + `TUREAD_DEV_BOOK=<书>` = 多库/書箱无头探针（**21 断言**，含移动/防成环）；
