...
| **阅读器跟随与镜像（2026-09-16 用户定并实施，未发版）** | 用户四条：① **目录/笔记的当前条目跟着阅读位置走，落在列表正中**（"我们读到哪里，哪个条目就处于目录的中间位置"，"这一点同样反应在笔记上"）② **挂载线下两挂件以页面中线镜像**（同宽、同高、离中线等距）+ 两处条目**放大** ③ **右键挂载菜单提高对比度**（用户："点击创建笔记的右键挂载整个容器太透明了"）④ **批注输入栏改形态**：底部居中、纯色带边框、**零文字**（无标题/提示词/按钮），操作逻辑不变。另：**笔记按阅读先后排序而非创建时间**（用户定）—— 修掉 `createMark` 把新笔记**追加到末尾**的缺陷，排序权威上收领域层 `compareNoteOrder`（存储与 UI 共用一份）。判据 = `STYLE.md` §5.8/§5.2 例外⑥；实现 = `components/readerFollow.ts`（纯函数 + 单测）、`ReaderRail`/`TocPanel`/`NotesPanel`/`NoteComposer`/`styles.css` | `client/docs/STYLE.md` §5.8 + §10 v1.8；`FEATURES.md` §11；`client/src/renderer/src/components/readerFollow.ts` |
...
**★ 现在在哪（2026-09-16 之二）· 阅读器「跟随与镜像」（未发版，真机验收待用户走）**
- 权威：**视觉与几何 = `STYLE.md` §5.8（新增「左右镜像」「跟随当前位置」「右键挂载菜单」「批注输入栏」四行）
  + §5.2 例外⑥ + §10 v1.8**；功能描述 = `FEATURES.md` §11；决策汇总 = §3 的「阅读器跟随与镜像」行；
  未做项 = `TODO.md`（PDF 翻页专题 / 章内精确化 / 当前条目无视觉标记）。
- 实现落点：`components/readerFollow.ts`（**判据纯函数 + 单测**）｜`ReaderRail`（镜像线段 + `__lead` 占位、
  `activeChapter` 透传）｜`TocPanel` / `NotesPanel`（挂载即落位 + 跨章平滑跟随）｜`NoteComposer`（底部居中零文字）｜
  `styles.css`（`--rail-*` 三件 + 两条使用处算式、菜单/输入栏对比度、条目放大 15px）｜
  `domain/anchor.ts` 的 **`compareNoteOrder`**（排序权威上收；`sqliteStore` 与 `ReaderFeature` 共用一份）。
- 验证（本轮实测）：`typecheck:all` **四 project 全绿**；`npm test` **98 断言**（新增 `readerFollow` 7 +
  `compareNoteOrder` 5）；样式样张 `smoke.cjs` **新增三条几何机检全绿**（`mirrorOk` 两侧 gap 282/282px、
  同宽 140px、高度上限同值；`followedOk` 列表真能滚 475px + 当前条目偏差 **1px**；`composerOk` 零可见文字、
  无 placeholder、非透明底、发丝描边、水平居中 0px、挂载即聚焦）；真机探针三连 **无回归** ——
  `note` **全 PASS**、`library` **50 行 PASS**（文档旧记"56 断言"未复核口径，**登记为观察**）、
  渲染自检 `[TUREAD-TEST-OK][dev] 渲染OK …章節數=98 …恢復=ok`。
  ⚠ **本轮新增两处"量法坑"**（会造成**假 FAIL**，已写进 `smoke.cjs` 注释与 STYLE §8.0）：描边宽度不能断言字面
  `1px`（DPR 1.25 → 实测 0.8px）、居中要跟 `documentElement.clientWidth` 比（`innerWidth` 含滚动条）。
- ⏳ **本次没做**：① **PDF 翻页专题**（用户定调"以后作为一个专题去聊"→ 只登记）；② 跟随居中的**章内精确化**
  （现在只到章）；③ 当前条目**没有视觉标记**（用户未要求，动手前先问）。
...
⚠ **2026-09-16 本轮复跑：仍是 3/3 命中**（同一本书、隔离 userData）—— 与"约 1/4"的历史观测相比
本次更稳定地复现，**但本轮改动与该路径无因果关系**（未触碰 `open` 的代次守卫与 `getLastRead` 竞态）；
这条旧账仍是**唯一**能让 `note` 探针跳过 ⑦⑧ 断言的原因。
