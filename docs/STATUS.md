...
| 版本 | 日期 | 内容 |
|---|---|---|
| v0.1.14 | 2026-09-13 | **沉浸全屏 + 交互根修 + 数据地基**：① **阅读器交互三则**：参数面板**默认展开**（废"默认收起"）/ 布局模式移入面板（滾動/單頁/雙頁，改 = 原地重开生效，`readerSettings` 唯一写者，设置页不再写）/ 空目录垂挂「本書沒有目錄索引」占位；TODO 开设 **PDF 专区**（旋转立项/填充策略备注/夜间反相集中管理）并瘦身（已完成项移出待办）② **沉浸全屏**：`F11`（意图层收编）+ 设置开关（默认关）+ 标题栏动画收起（高度+透明度 240ms，消闪烁）+ `Esc` 分流（面板→全屏→退阅读器）+ 离开阅读器自动还原窗口 ③ **iframe 事件桥**（`components/iframeBridge.ts`）：键盘/滚轮被文档边界挡在 iframe doc 上的**根修**（用户实测点进书页后 F11 失灵）——递归扫描+递归观察同源 iframe 文档（覆盖 PDF 每页子 iframe），按键**只看界面不看焦点**，输入类让位 ④ **分页模式滚轮翻页**（默认键盘、无按钮控件，像 koodo）：累积+冷却窗防连翻，滚轮接管上移到整个阅读器区（消左右点击带死区，目录/参数面板让位）⑤ **PDF 改纸宽 = 原地重开填充**（用户定"应当填充"；kookit PdfRender 无重排入口，复用 reopenTick 机制）⑥ 挂载线 28→44px；**SQLite spike 全绿**（better-sqlite3@12 + electron prebuild + asarUnpack，dev 与打包产物双验证，328 行事务批量写 2-3ms，原生路线成立，`npm run rebuild:sqlite` 脚手架）⑦ **定位转换机制**设计提案并批复（DATA_MODEL §3.1.1：统一信封+引擎载荷，quote 归锚点侧，compare 先 Fragment 后 Norm）⑧ TODO 登记 Veil 优化与 wasm 适用面评估。验证：typecheck 双绿；`paged-interact` 探针实证 EPUB 单页键盘/滚轮/目录三路正常；打包产物 SQLite spike OK（结果落盘验证）。⚠ 分页模式交互用户复测仍异常，待复现细节（TODO 渲染组）。**⑨ tag 后增补（同版本，未发版）——SQLite 单库落地（DATA_MODEL v2 进入实施）**：`SqliteStore` 主进程适配器替代 JsonStore（schema 按 DATA_MODEL §2 全量建表，containers/notes 表先立未接 UI；books 增 `metadata` JSON 列 + `meta` 簿记表）；**one-shot 迁移器**（library.json/config.json → turead.db，meta 标记防复活，旧文件改名 `.migrated` 留档）；`TUREAD_USER_DATA` 环境变量支持独立 userData（无头验证不污染真实书库）；**销案**：适配器导航方法静默 no-op（未打开即抛错，与 renderTo 同口径，pageTurn/goToChapter 调用方补日志）、AppShell navigate 恢复分支缺 catch（候选①补上）、JsonStore 批量导入写放大（存储层已换 SQLite，问题不复存在）；标题栏搜索栏加大（h-7/w-420）+ 占位去「Ctrl+F」提示（用户定：快捷键属于说明书不属于界面）。验证：typecheck 双绿；独立 userData 下 327 本真实书库迁移 + EPUB 无头自检全绿（恢复=ok/位置回写入库）+ 二次启动不重复迁移（`node:sqlite` 直查库确认 327 行/3 设置键/标记位）。**⑩ tag 后增补（同版本，未发版）——多书库落地 + 交互两修（用户 2026-09-13 定）**：**多书库**（`LibraryManager` 主进程库管理器）：`config.json` 正式降级为**引导文件**（库注册表 + 当前库 id，不再存任何书库数据），一库 = 一 .db；底部状态栏「導入」右侧新增**「書庫」菜单**（与既有文字按钮同款：库列表·当前标「當前」/ 点击切换 / 新建書庫，切换成功 main 广播 `store:library-changed`，AppShell 清选中与阅读器状态、LibraryFeature 重载书单与库级设置）；`ILibraryStore` 增库管理三方法（契约 v0.3.5）；修 `SqliteStore` close 后 init 不重开的切库 bug；**Esc 分流纠正**（用户定：参数面板是常伴工具，Esc 收面板是错误设计——Esc 只做 全屏→退阅读器）；**搜索栏修复**（外层 pointer-events-none 连坐输入框，点击穿透无法聚焦——输入框补 pointer-events-auto）。验证：typecheck 双绿；`library` 探针（`TUREAD_DEV_PROBE=library`）7 断言全过（新建→空库→独立导入→切回书单恢复→注册表正确），二次启动不重复迁移；真实书库已在用户实机上完成迁移（turead.db + .migrated 留档）。**⑪ tag 后增补（同版本，未发版）——書庫管理弹窗（用户 2026-09-13 定：参考 Obsidian 仓库管理页）**：状态栏「書庫」改开**管理弹窗**（替换下拉菜单）：库列表（点名切换，当前标「當前」）/ **新建**（行内输入命名，即建即切）/ **更名**（行内编辑，`renameLibrary` 契约 v0.3.6，显示名与 dbPath 解耦）/ **所在文件夾**（`fs:show-in-folder`，系统文件管理器高亮 .db——路径管理在主进程，渲染层不可指定路径）。**用户同批立项双登记**（TODO）：跨库数据分歧（同书多库各持进度/笔记，随笔记落地显性化，需定共享模型）、书库双模式（虚拟映射=照搬真实文件夹默认遍历子文件夹、一库一个、书箱≈真实文件夹；自建书箱=自建树、右键书架空白处新建書箱）——数据层已就绪（containers/container_books 表），交互形态待定。验证：typecheck 双绿；`library` 探针 8 断言全过（增更名断言）。**⑫ tag 后增补（同版本，未发版）——书库双模式落地（用户 2026-09-13 定稿交互）**：**建库二选一**（管理弹窗新建时选模式：虛擬映射 = 选定唯一根文件夹、建库即递归扫描导入；自建書箱 = 空库；`LibraryEntry.mode/rootPath` 入引导文件，`LibraryManager.createLibrary(name, mode, rootPath)`）；**资源管理器式层级浏览**：書箱/文件夹与书籍外观相似（`ContainerItem`，单击进入下一层）、虚拟映射的层级 = 真实文件夹树（`fsListDirectories` 列子目录，书 = 直接位于当前层的）、自建模式 = 書箱树（containers 表 CRUD 落地：create/rename/remove/list + `listBooksAtLevel` 层级取书，根层 = 未入箱的书）；状态栏右端**当前层级面包屑**（自建 = 書箱名、映射 = 文件夹名，可点回跳）；**右键书架空白处 → 新建書箱**（行内命名，`ContextMenu` 组件）；**拖拽外部文件进窗口 = 导入当前书库**（preload `webUtils.getPathForFile` 取真实路径，非书格式忽略；Electron 须全局 preventDefault dragover/drop 防窗口导航）。验证：typecheck 双绿；`library` 探针 13 断言全过（書箱 CRUD/层级取书/映射库持久化）；EPUB 无头自检全绿无回归。⚠ **可见性与追踪**（虚拟映射的书被外部更改/移除怎么办）= 用户提出的待讨论项（TODO）。**⑬ tag 后增补（同版本，未发版）——書箱交互按资源管理器语义重做（用户 2026-09-13 纠正）**：**書箱 = 与书籍同地位的一等条目**（此前误做成半成品过滤器）：单击选中/双击进入/Enter 进入/F2 更名（Enter 或失焦提交，**不用 Esc**——用户定：建箱不是进界面，Esc 逻辑讲不通）/右键菜单（書箱：新建子書箱/更名/移除；书：打開/詳情/從書庫移除；空白：新建書箱 = **立即建出「新建書箱」并直接行内更名**，废"大输入框占位"）/拖书入箱 = **移动**（单亲归属 `moveBookToContainer`，拖到面包屑段 = 移到该层，拖到根 = 移出）。修复：更名 Enter 冒泡导致"提交即进入"（更名中条目不响应条目级键盘 + 输入框 stopPropagation）；**书架内容区 select-none**（用户定：键鼠标准化 = 清出原生文字选择干扰）。验证：**computer-use 实机全链路**（独立 userData 真实点击：右键建箱 → 行内更名「齐泽克」→ 拖书入箱 → 双击进入確認箱内书籍）+ `library` 探针 16 断言全过（含 moveBookToContainer 移动/还原）+ typecheck 双绿 |
