...
| **阅读器功能边界三判断（2026-09-12 用户定，产品哲学层）** | ① **阅读器=索引，不做实体管理**：分类/容器/标签不自建，留给用户选择的专门资产管理程序（TuRead 不依赖、不绑定、不推荐任何外部程序——只是自己不做，把这部分让位给用户已有的工具）——「多书架/书库容器」TODO 据此冻结重审；② **暴露的参数是负担**：默认暴露集必须最小，新参数不自动进面板（「剩余阅读参数」降速）；③ **阅读界面=注意力焦点模式**：不做功能抽屉，不用的功能透明度逐渐提高（淡出）——淡出要慢、召回路径恒定、仅限阅读页。效力高于行业惯例；待吸收进 `STYLE.md` §5.8 | `docs/LANDSCAPE.md` §6 |
