...
| **阅读器跟随与镜像（2026-09-16 用户定；**同日 v1.9 纠正几何口径**）** | 用户四条：① **目录/笔记的当前条目跟着阅读位置走，落在列表正中**（"我们读到哪里，哪个条目就处于目录的中间位置"，"这一点同样反应在笔记上"）② **挂载线下两挂件关于「挂载线的中心」镜像**（同宽、同高）③ **右键挂载菜单提高对比度**（用户："点击创建笔记的右键挂载整个容器太透明了"）④ **批注输入栏改形态**：底部居中、纯色带边框、**零文字**（无标题/提示词/按钮），操作逻辑不变。另：**笔记按阅读先后排序而非创建时间**（用户定）—— 排序权威上收领域层 `compareNoteOrder`（存储与 UI 共用一份）。⚠ **v1.9 纠正（用户驳回 v1.8）**：v1.8 把两段宽/位置写成**纸宽的函数**（"整个挂载线变成了相对于中间显示内容的绝对长度，这点更是可笑之极，谁让你改的？"）→ 改回**线是固定的一条**、两段**等宽且各贴线的一端**（等宽+贴齐 ⟺ 互为镜像，无中心点算式）；批注输入栏同时改为**单行高 + 纸宽×0.9**，并把**生命周期绑到它的对象**（选区型随选区消失而关；**笔记型随那条笔记从库里消失而关**）；**单击正文高亮 = 看内容（左侧定位）而非改内容**。判据 = `STYLE.md` §5.8/§5.2 例外⑥ + §10 v1.9；实现 = `components/readerFollow.ts`（纯函数 + 单测）、`ReaderRail`/`TocPanel`/`NotesPanel`/`NoteComposer`/`styles.css` | `client/docs/STYLE.md` §5.8 + §10 v1.9；`FEATURES.md` §11；`client/src/renderer/src/components/readerFollow.ts` |
...
**★ 现在在哪（2026-09-16 之二 / **v1.9**）· 阅读器「跟随与镜像」（未发版，真机验收待用户走）**
- 权威：**视觉与几何 = `STYLE.md` §5.8（「挂载线」「左右镜像」「跟随当前位置」「右键挂载菜单」「批注输入栏」）
  + §5.2 例外⑥ + §10 **v1.9****；功能描述 = `FEATURES.md` §11；决策汇总 = §3 的「阅读器跟随与镜像」行 +
  「右键菜单坐标」行；未做项 = `TODO.md`（PDF 翻页专题 / 章内精确化 / 当前条目无视觉标记 / 点高亮的"看内容"形态）。
- 实现落点：`components/readerFollow.ts`（**判据纯函数 + 单测**）｜`ReaderRail`（两段等宽贴线两端、`activeChapter`
  / `focusNote` 透传）｜`TocPanel` / `NotesPanel`（挂载即落位 + 跨章平滑跟随 + 「看这条」定位）｜
  `NoteComposer`（底部居中、零文字、单行、纸宽×0.9）｜`styles.css`（`--rail-panel-w` / `--rail-panel-h`、
  菜单/输入栏对比度、条目放大 15px）｜`domain/anchor.ts` 的 **`compareNoteOrder`**（排序权威上收）｜
  **适配器 `hostOffset()`**（右键/选区/点击高亮的宿主坐标换算，逐层累加 iframe 偏移）。
- ⚠ **v1.8 → v1.9 的纠正（user 驳回两条，我改错的两条）**：① 两段宽/位置**不许**由纸宽派生（v1.8 那样写 =
  "线的长度成了纸的函数"，用户判定不可接受）→ 线固定、两段等宽且各贴线的一端；② 批注输入栏的**宽高**
  （单行 + 纸宽×0.9）与**生命周期**（绑在它的对象上：选区型随选区消失、**笔记型随笔记消失**）、
  ③ **单击高亮 = 看内容而非改内容**。
- **另修 1 个真 bug（v1.8 之前就存在）**：右键菜单坐标**从未换算**成宿主坐标 → 菜单出现在非内容处
  （用户："非常可笑"）；⚠ 旧探针把 bug **断言成了契约**（`ctx.x === 240`），这次一并纠正为
  "换算后的宿主坐标 + 必须落在纸内"。
- 验证（本轮实测）：`typecheck:all` **四 project 全绿**；`npm test` **98 断言**（新增 `readerFollow` 7 +
  `compareNoteOrder` 5）；样式样张 `smoke.cjs` **四条几何机检全绿**（`mirrorOk` 两段同宽 280px + 两侧贴齐 0px +
  高度上限同值；`widthIndependentOk` 现场把纸宽 260→520 后两段宽度**一点不变**；`followedOk` 列表真能滚 475px +
  当前条目偏差 **1px**；`composerOk` 零可见文字 + 无 placeholder + 非透明底 + 发丝描边 + 居中 0px +
  **宽偏差 0** + 高 **45px** + 挂载即聚焦）；真机探针 —— `note` **全 PASS**（含新增：右键点落在纸内、
  输入框宽 = 纸宽×0.9 与纸同轴、高 44.9px、**选区消失→输入框消失**、Enter 提交真的落库 0→1、
  笔记面板行数与库同口径、重开后自动回挂 span=1）、`library` **50 行 PASS**（文档旧记"56 断言"未复核口径，
  登记为观察）、渲染自检 `[TUREAD-TEST-OK][dev] 渲染OK …章節數=98 …恢復=ok`。
  ⚠ **三处"量法/分段"教训**（都会造成**假绿或假 FAIL**）：① 描边宽度不能断言字面 `1px`（DPR 1.25 → 实测 0.8px）；
  ② 居中要跟 `documentElement.clientWidth` 比（`innerWidth` 含滚动条会判偏 5~8px）；
  ③ **探针分段也是判据** —— `noteProbe` 原把 ⑦⑧ 挂在"重开成功"上，重开一命中就整段被跳过（判据等于不存在），
  已重排为"只有重开后自动回挂依赖重开"。
- ⏳ **本次没做**：① **PDF 翻页专题**（用户定调"以后作为一个专题去聊"→ 只登记）；② 跟随居中的**章内精确化**
  （现在只到章）；③ 当前条目**没有视觉标记**（用户未要求，动手前先问）；④ 点高亮的"看内容"最终形态
  （现为左侧定位；只读浮层候选）。
