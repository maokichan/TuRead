...
**★ 当前主目标（2026-09-16 起）· 笔记管理工具组件 —— 契约与视觉已立案，待实施**
- 形态裁决见 §3 决策表第 1 行；**视觉与交互立案见第 2 行**。权威 = `FEATURES.md` **§12**（形态/交互）+
  `STYLE.md` **§5.10**（v1.5 视觉语汇；连带修订 §5.1 三角负片、§5.2 例外⑤）+
  `CONTRACTS.md` **v0.4.1**（§4.4 读模型）/ **v0.4.2**（`NoteView`/`NoteFilter`/`NoteTextFocus`/`NoteSettings` + §4.6 `IClipboard`）+
  `DATA_MODEL.md` **§5 问题 6**（检索批复）。
- **代码一行未动**（契约先行）：实施按序 ①~⑦，落在 `TODO.md` 的「笔记管理」条目下。
- ⚠ 三个前置：① **视觉已立案**（可动 UI 了）；② `FeatureHost.openReader` 扩**可选**参数
  （向后兼容 —— Library / Room / 四个 dev 探针的既有调用不变）；③ **瀑布流 × `useVirtualRange`
  的等高 `stride` 假设冲突**要先量化再定（`STYLE.md` §5.10 末段）。
