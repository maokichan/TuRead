...
> **更新 2026-09-17 · client v0.2.0 发版（房间功能上线）**：用户定调"临时暂停其他一切功能开发，直接上线
> 房间功能"，本轮落成 **阅读器聊天室**（挂载线右段两格：参數 / 聊天）并**首次把房间同步与真实 server
> 打通**（根因 = 连接模型与服务器文本不一致，见 §6「本轮落成」）。**用户当晚真机双开验收通过**
> （两个 client 同房共读那本 81.7MB 的 PDF：成员 2 人、位置上报、双向聊天全通），随后指示滚 **0.2.0**。
> 契约 = `CONTRACTS.md` **v0.4.4**，视觉 = `STYLE.md` §5.8 **v1.12**，交接 = §6。**未打包**（打包另派）。
...
同步服务器用 Go，**v0.2.0 已实现**（`server/`）；**client v0.2.0**。
...
- **tag 约定**：两端版本号独立滚动（server `v0.2.x` / client `v0.2.x`），tag 带端名前缀：`client-v0.2.0`
...
| 版本 | 日期 | 内容 |
|---|---|---|
| **v0.2.0** | 2026-09-17 | **房间功能上线（阅读器聊天室）+ 房间同步首次与真实 server 打通**。① **根因修复**：服务器 WS **握手就要 `?room=&nick=`**（`transport/ws.go`，缺任一/昵称 >12 字直接关连接），而客户端旧模型是"先连一条通用连接、再发 room.join" → **在服务器上根本走不到 join**（"房间是半成品"的直接原因）→ 改为**按房间建连**：`INetService.connect` 语义收窄为"建立会话（只领成员 token）"，新增 `openRoom/closeRoom`；`IRoomSession.joinRoom` 等 ack 加 **10s 超时**、并发 join 不再覆盖式挂起、断线重连后**重发 join + `after=` 增量补拉**；REST 补超时；`emitLocation` 与自动路径统一节流出口；`JoinResult` 收敛到领域层。② **协议词汇收拢** `core/domain/protocol.ts`（type 常量 / **reason 容错归一** / 昵称约束 / 握手地址 / 历史路径 / 聊天合并 + 15 单测）—— 起因是**实测的服务器-文档偏差**：服务器下发 `"book mismatch"`（**空格分词**），旧客户端按连字符精确匹配 → "书不匹配"被静默降级成 server-error。③ **阅读器聊天室**：挂载线**右段两格**「閱讀參數 / 聊天」（与左段目錄/筆記同构；`.left-tabs` → `.rail-tabs`），**只在经房间进入的那本书上存在**（"沒有通過房間進入一本書就不會有這個項"），消息与「房間」组件**同一份**（`features/roomSession.ts`），`c` 键切换。④ **无书成员加入**（`joinRoom(book\\|null)` → `fingerprint: null`）：服务器本就支持（`API.md`「room.join」），客户端用例层补齐（契约 v0.4.4）。**验证**：`typecheck:all` 四 project 全绿；`npm test` **113 断言**（98 → +15）；样式样张 `smoke.cjs` **八条几何机检全绿**（新增 `chatDrawerOk` / `noRoomTabsOk`）；真机 **`TUREAD_DEV_PROBE=room` 对真 server 16 条断言全 PASS**（新增 `dev/roomProbe.ts`）；**用户当晚双开验收通过**（两 client 同房：成员 2 人 / 位置上报 / 双向聊天）。⚠ 未打包；⚠ 遗留：服务器侧 reason 字符串与 `POST /rooms` 昵称长度校验（`TODO.md` 同步组）、多人场景的"成员离开"与正式双 token 方案 |
...
## 6. 交接快照（2026-09-17 · **client v0.2.0 已发版**）
...
> **验收**：用户当晚**用两个 client 同房实测通过**（同一本 81.7MB PDF：成员 2 人、位置上报、双向聊天），
> 随后指示 **滚 0.2.0**；**未打包**（打包另派，见 §2）。

**★ 本轮落成（契约 = `CONTRACTS.md` v0.4.4，视觉 = `STYLE.md` §5.8 v1.12）**
...
5. **无书成员加入**（v0.4.4）：`joinRoom(roomId, book | null)` → `fingerprint: null`（服务器 `API.md` 本就支持，
   客户端用例层原来表达不了）；位置比较的格式键退化为从 join-ack 的 edition 扩展名推。
   **UI 入口仍未做**（要产品形态，见 `TODO.md`）。
6. **验证**：`typecheck:all` 四 project 全绿；`npm test` **113 断言**（+15）；样式样张 `smoke.cjs`
   **八条几何机检全绿**（新增 `chatDrawerOk` / `noRoomTabsOk`）；**真机房间探针对真 server 16 条断言全 PASS**；
   **用户双开验收通过**。⚠ 服务器侧两处**未修**（本轮只做客户端）：reason 字符串与 `API.md` 不一致、
   `POST /rooms` 的 `owner` 昵称无长度校验 —— 已登记 `TODO.md` 同步组，**答复见下面「人类开发者提问」**。
...
**工作区状态**：client **v0.2.0 已提交并打 tag `client-v0.2.0`**；`client/package.json` = **0.2.0**；
client 工作树干净。⚠ **server 侧三处未提交改动来源不明**（`cmd/server/main.go`、`internal/room/manager.go`、
`internal/store/store.go` —— 其中 `manager.go` 就是下面那条提问所在），**本轮未动 server**，
下次动 server 前先与用户确认。kookit 子模块的 `m` 是其自身工作树噪音，**勿动**。
**发行物**：`client/release/` 里仍只有 v0.1.15 那次便携版；**v0.1.16 ~ v0.2.0 全部未打包**（打包另派 agent）。

**★ 下一步（按建议优先级；唯一待办清单在 `TODO.md`）**
1. **服务器侧两处对齐**（`TODO.md` 同步组前两条）：reason 字符串与 `API.md` 不一致、`POST /rooms` 昵称长度校验。
2. **多人场景补验 + 正式的多端身份方案**（`TODO.md`）：成员离开后不残留、双 token 的正式做法
   （服务器按 IP 签发 token + 同 token 踢旧连接 = 同机多开必互相踢，这是 `API.md` 登记的 NAT 限制）。
3. **无书成员的 UI 入口**（用例层已就绪）：进房后按 edition 下载副本 → 落盘 → 导入 → 打开；
   以及"进房前没有这本书"的选择流程。
...
⑥ **聊天室的手感**（见 `TODO.md` 的"待真机复看"条目：自动滚到底 / 长消息下的遮罩浓淡 / 输入框自增长 / `c` 键）。
...
（库是库内实体，涉及收录/书箱的调用显式带 `libraryId`）｜**契约版本 v0.4.4**（`CONTRACTS.md` §8）｜
