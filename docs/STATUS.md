...
- **v0.1.9 后续修正（2026-09-08）**：① 视图切换改为**单个文字按钮**（显示当前视图名 `列表`/`網格`，
  点击后视图与文字同时切换，动画中 42% 处替换文字）；导入按钮改**文字按钮**「導入」；
  两者均为 18px 加粗源流明体、无边框。② **修复"进入阅读器恢复上次内容"未生效** ——
  侧边栏按钮原先绕过 `host.navigate`（恢复逻辑所在处），现已统一走 `host.navigate`；
  自检断言改为点击**真实侧边栏按钮**。验证：typecheck 全绿；EPUB/PDF 自检 `恢复=ok`。
